...
class Listing:
    def __init__(self, name, location, property_type, accommodates,
                 ...
                 review_rating, tags, listing_id: int):
...
def load_listings(filename=Listings_File):
    ...
    # --- ROBUST COLUMN RENAMING LOGIC (THE FIX) ---
    ...
    for col_name in price_alternatives:
        if col_name in df.columns:
            found_price_col = col_name
            break
    
    if found_price_col:
        df.rename(columns={found_price_col: 'price'}, inplace=True)
    else:
        # If no price column is found after checking all alternatives, raise an error.
        raise ValueError(f"CRITICAL ERROR: Could not find a price column in '{filename}'. Checked for: {price_alternatives}")

    # You can add similar logic for other important columns if needed
    # For example, standardizing the rating column:
    rating_alternatives = ['review_rating', 'rating', 'reviews_rating', 'Review Rating']
    found_rating_col = None
    for col_name in rating_alternatives:
        if col_name in df.columns:
            found_rating_col = col_name
            break
    if found_rating_col:
        df.rename(columns={found_rating_col: 'review_rating'}, inplace=True)

    # Fill any potential missing values (NaN) with safe defaults AFTER renaming
    df['price'].fillna(0.0, inplace=True)
    if 'review_rating' in df.columns:
        df['review_rating'].fillna(3.0, inplace=True)
    # ----------------------------------------------------

    # Convert the cleaned DataFrame to a list of Listing objects
    listings = [
        Listing(
            listing_id=i, # Use enumerate index as a stable ID
            ...
            min_nights=row.get("min_nights", 1),
            max_nights=row.get("max_nights", 30),
            review_rating=row.get("review_rating", 3.0),
            tags=row.get("tags", "")
        )
